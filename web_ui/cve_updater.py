...
class CVEUpdater:
    ...
    def fetch_nuclei_templates(self) -> Dict:
        ...
        try:
            ...
            response = requests.get(self.nuclei_templates_url, timeout=30)
                            ...
                            for template in templates:
                                if template.get('type') == 'file' and template.get('name').endswith('.yaml'):
                                    ...
                                    if 'CVE-' in cve_match.upper():
                                        ...
                                        if download_url:
                                            template_content = requests.get(download_url, timeout=10).text
    ...
    def fetch_metasploit_modules(self) -> Dict:
        ...
        try:
            ...
            response = requests.get(self.metasploit_search_url, timeout=30)
    ...
    def fetch_exploitdb(self) -> Dict:
        ...
        try:
            ...
            response = requests.get(edb_url, timeout=30)
...
# Auto-update on import (if database is older than 24 hours)
if os.path.exists(cve_updater.db_path):
    mod_time = datetime.fromtimestamp(os.path.getmtime(cve_updater.db_path))
    if datetime.now() - mod_time > timedelta(hours=24):
        print("CVE database is outdated. Updating...")
        cve_updater.update_all_sources()
else:
    print("No CVE database found. Creating initial database...")
    cve_updater.update_all_sources()
